...
import com.dsadeghi.stackqueue.ArrayListStack;
...
public class PalindromeTest {
    ...
    private boolean isPalindrome(String s) {
        ...
        //Special case if string is empty or one character
        if (charStack.size() == 0 || charStack.size() == 1) {
            return true;
        }

        ArrayListStack<Character> charStack2 = new ArrayListStack<>();

        {
            int stackSize = charStack.size();
            for (int i = 0; i < stackSize / 2; i++) {
                charStack2.push(charStack.pop());
            }
        }
        ...
        while(!charStack.empty()) {
            if (!charStack.pop().equals(charStack2.pop())) {
                return false;
            }
        }
